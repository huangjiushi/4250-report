...
```{r}
y[ij] = Observation from subject j in group i
n[i] = Number of subjects in group i
N = n[1] + n[2] + ... + n[g] = Total sample size

bar(y[i.]) = frac(1,n[i])sum(Y[ij],j=1,n[i]) = Sample mean for group i
bar(y[..]) = fram(1,N)sum(sum(Y[ij],j=1,n[i]),i=1,g) = Grand mean
 
Sum of squares between groups = sum(n[i](bar(y[i.])-bar(y[..]))^2,i=1,g)
Sum of squares within groups = sum(sum((Y[ij]-bar(y[i.]))^2,j=1,n[i]),i=1,g)
```
